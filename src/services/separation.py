...
class SeparationService:
    def __init__(self):
        ...
        logger.info(f'Separation Service initialized using: {self.device.upper()}')
    ...
    def run_separation(self, input_path, output_root, progress_callback=None):
        ...
        try: 
            ...
            logger.error(f"Demucs AI Error: {e}")
            raise Exception("AI separation failed. Check terminal for details.")
    ...
    def _post_process_stems(self, stems_folder):
        ...
        for file_name in wanted_files:
            ...
            if stem_path.exists():
                logger.info(f"Cleaning & Converting to Mono: {file_name}")
